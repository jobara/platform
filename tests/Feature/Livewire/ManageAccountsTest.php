...
test('accounts can be searched', function () {
    livewire(ManageAccounts::class)
        ...
        ->assertSee($this->regulatedOrganization->name)
        ->assertSee('1 result')
        ->set('searchQuery', $this->individual->user->email)
        ->call('search')
        ->assertSee($this->individual->user->email)
        ->assertDontSee($this->organization->contact_person_email)
        ->assertDontSee($this->regulatedOrganization->contact_person_email)
        ->assertSee('1 result')
        ->set('searchQuery', $this->organization->contact_person_email)
        ->call('search')
        ->assertDontSee($this->individual->user->email)
        ->assertSee($this->organization->contact_person_email)
        ->assertDontSee($this->regulatedOrganization->contact_person_email)
        ->assertSee('1 result')
        ->set('searchQuery', $this->regulatedOrganization->contact_person_email)
        ->call('search')
        ->assertDontSee($this->individual->user->email)
        ->assertDontSee($this->organization->contact_person_email)
        ->assertSee($this->regulatedOrganization->contact_person_email)
        ->assertSee('1 result');
});
